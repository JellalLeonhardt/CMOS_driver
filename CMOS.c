          ...
          size_t count, loff_t *ppos)
{
    ...
    if (copy_to_user((void __user *)buf, (const void *)data, (unsigned long)(xferred+7)/8) != 0) { 
        return -EIO;
    }
           ...
           size_t count, loff_t *ppos)
{
    ...
    if (copy_from_user(kbuf,buf,(unsigned long)(count/8)+1)) {
        kfree(kbuf);
        return -EFAULT;
    }
...
/*
 * Ioctls to adjust and verify CRC16s.  因为ioctl弃用了 故而 需做更改
 */
//static int
...
/* 新版本的 cmos_ioctl */
static long
cmos_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    unsigned short crc = 0;
    unsigned char buf;
    switch (cmd) {
...
static struct file_operations cmos_fops = {
    ...
    //.ioctl = cmos_ioctl,            /* Ioctl method */  注意：v2.6.35到v2.6.36-rc1间，去掉了ioctl 而是 改用了unlocked_ioctl
